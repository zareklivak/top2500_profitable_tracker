...
const url = 'https://webhook.site/token/b5823812-8c4c-44bd-9749-37c8d03cb31f/requests';
const apiKey = '9a0a2acf-471e-4fc0-9ebd-0624001668c1';
...
function updateCoinStats(transaction) {
    if (transaction.tokenTransfers && transaction.tokenTransfers.length > 0 &&
        ...
        if (isAccountSpamming(account)) return;
...
async function processData(allData) {
    ...
    if (!Array.isArray(allData) || allData.length === 0) {
        logBox.log('No data to process or invalid data structure');
        return;
    }
    ...
    for (const item of allData) {
        if (item && item.method === 'POST' && !processedIds.has(item.uuid) && new Date(item.created_at).getTime() >= lastResetTime) {
            try {
                ...
                logBox.log('Error parsing transaction data: ' + parseError.message);
            }
        }
    }
...
function cleanupOldData() {
    const fiveMinutesAgo = Date.now() - 5 * 60 * 1000;
    for (const stats of coinStats.values()) {
...
async function startScanning() {
    ...
    setInterval(async () => {
        ...
        try {
            ...
            logBox.log('Error in scan cycle: ' + error);
        }
    }, 15000);
}
